...
# ~/CIS-LBK/functions/nix_ensure_permissions_motd_configured.sh
# 
...
nix_ensure_permissions_motd_configured()
{
	...
	nix_ensure_permissions_motd_configured_chk()
	{
		...
		l_buffer=$(stat /etc/motd | grep -i access | grep -i id)
		...
		if [ "$l_uid" != "$l_uid_correct" -a "$l_gid" != "$l_gid_correct" ]; then
			l_uid="failed"
		fi

		# If $l_perms does not equal "failed" and $l_uid does not equal "failed", we pass
		if [ "$l_perms" != "failed" -a "$l_uid" != "failed" ]; then
			...
			[ "$l_perms" != "$l_perms_correct" ] && echo "- /etc/motd permissions are set to \"$l_perms\"" | tee -a "$LOG" 2>> "$ELOG"
		   	[ "$l_uid" != "$l_uid_correct" -a "$l_gid" != "$l_gid_correct" ] && echo "- /etc/motd owner is \"$l_uid\" and group is \"$l_gid\"" | tee -a "$LOG" 2>> "$ELOG"
		   	echo "- End check - /etc/motd permissions and ownership" | tee -a "$LOG" 2>> "$ELOG"
		   	return "${XCCDF_RESULT_FAIL:-102}"
		fi					
	}

	nix_ensure_permissions_motd_configured_fix()
	{
	...
	if [ "$?" = "101" ]; then
		...
		nix_ensure_permissions_issue_configured_fix
		nix_ensure_permissions_issue_configured_chk
		if [ "$?" = "101" ]; then
